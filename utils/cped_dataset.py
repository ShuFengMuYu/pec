...
class CpedDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 目前不进行转换
        history_speaker_tokens = history_speaker_types

        
        # # 创建历史对话对应的性别嵌入
        # history_gender_types = data_index['Gender'].tolist()[-max_history_utterances:]
        # # 创建历史对话对应的年龄嵌入
        # history_age_types = data_index['Age'].tolist()[-max_history_utterances:]

        # 创建历史对话角色嵌入
        history_persona_types = {}
        for persona in self.persona_type:
            history_persona_types[persona] = data_index[persona].tolist()[-max_history_utterances:]
        ...
        if self.with_emotion: # 需要把情感标签加进词表！
            ...
            # 目前
            history_emotion_tokens = history_emotion_tokens
        else:
        ...
        # 目前不进行tokenize
        current_emotion_token = data_index[self.emotion_type].tolist()[-1]
    ...
    def process(self, 
        ...
        # 原本
        # bos [speaker1] t1 t2 t3 [speaker2] t4 t5 t6 [eos]
        ...
        # 修改后
        # bos t1 t2 t3 [eos] t4 t5 t6 [eos]
        sequence = [[bos]] + history_utterance_tokens + [response_utterance_tokens]
        sequence = [sequence[0]] + [s + [eos]
                                    for i, s in enumerate(sequence[1:])] 
        
        # instance["age"] = [bos] + [history_age_types[i] for i, s in
        #                                        enumerate(sequence[1:])
        #                                        for _ in s]
        # instance["gender"] = [bos] + [history_gender_types[i] for i, s in
        #                                        enumerate(sequence[1:])
        #                                        for _ in s]
        
        for persona in self.persona_type:
            if instance.get(persona) is not None:
                raise RuntimeError('属性已经存在')
            instance[persona] = [CPED_SPECIAL_TOKENS[0]] + [history_persona_types[persona][i] for i, s in
                                               enumerate(sequence[1:])
                                               for _ in s]
        
        instance["input_ids"] = list(chain(*sequence))
        instance["token_type_ids"] = [CPED_SPECIAL_TOKENS[0]] + [speaker_tokens[i] for i, s in
                                              enumerate(sequence[1:])
                                              for _ in s]
        ...
        if self.with_emotion:
            instance["emotion_ids"] = [CPED_SPECIAL_TOKENS[0]] + [emotion_tokens[i] for i, s in
                                               enumerate(sequence[1:])
                                               for _ in s]
            # only set the emotion in [speaker1] or [speaker2]
            if self.set_eda_in_speaker:
                instance["emotion_ids"] = set_emotion_in_speaker(instance["emotion_ids"],instance["input_ids"],bos, eos, speaker1, speaker2, self.pad)
